...
const Results = () => {
  ...
  useEffect(() => {
    ...
    const scholarshipStep = Math.ceil(
      result.scholarship / (duration / incrementTime),
    );
    ...
    const interval = setInterval(() => {
      ...
      if (scholarshipStart >= result.scholarship)
        scholarshipStart = result.scholarship;

      setScore(scoreStart);
      setScholarship(scholarshipStart);

      if (
        scoreStart === result.score &&
        scholarshipStart === result.scholarship
      ) {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center mt-8 md:mt-10 p-6 text-center">
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full md:mb-12 mb-8">
          ...
          <div
            className={`md:p-8 p-4 rounded-3xl border flex flex-col items-center ${
              isEligible
          ...
          >
            ...
            <div className="flex items-center gap-4 space-x-2">
              <Award
                className={isEligible ? "text-indigo-400" : "text-red-400"}
              />
              <span
