...
const Assessment = ({ phone, language }) => {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-12 text-center bg-slate-950">
        <div className="glass-card p-12 max-w-lg w-full border-red-500/30">
          <h2 className="text-4xl font-black mb-4 text-white">No Questions Available</h2>
          <p className="text-gray-400 mb-8 leading-relaxed">We couldn't find any questions for your selected group. The administrator might be updating the question bank.</p>
          <button onClick={() => navigate('/')} className="btn-primary py-4 px-12 font-bold w-full">
            Back to Home
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen flex mt-20 flex-col items-center justify-center p-4 sm:p-12 relative overflow-hidden">
      <div className="absolute top-12 md:top-10 left-4 md:left-10 right-10 h-2 bg-gray-800 rounded-full overflow-hidden">
          <motion.div 
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="flex justify-between items-center mb-10">
                <span className="text-indigo-400 font-bold uppercase tracking-widest text-sm">Question {currentIndex + 1} of {questions.length}</span>
            </div>
            
            <h3 className="text-xl sm:text-3xl font-semibold mb-4 md:mb-12">{currentQ?.text}</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {currentQ?.options.map((opt, idx) => (
                ...
                >
                  <span className="inline-block w-8 h-8 rounded-full bg-white/10 mr-4 text-center leading-8 font-bold">{String.fromCharCode(65 + idx)}</span>
                  {opt}
                </button>
              ))}
            </div>
