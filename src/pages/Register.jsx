...
const Register = ({ onNext }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(err.response?.data?.detail || 'An error occurred during registration.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 sm:p-12">
      <motion.div 
        ...
        className="glass-card p-8 sm:p-12 max-w-lg w-full border rounded-t-xl border-indigo-500/30"
      >
        <h2 className="md:text-3xl font-bold text-center font-medium text-[20px] mb-8">Registration</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="flex flex-col">
            <label className="text-16 font-medium mb-2 text-gray-400">Language for Assessment</label>
            <div className="flex gap-4 mb-4">
              ...
              >
               <span className='hidden md:block'>தமிழ் (Tamil)</span>  <span className='md:hidden'>தமிழ்</span>
              </button>
            </div>
          </div>

          <div className="flex flex-col">
            <label className="text-[16px] font-medium mb-2 text-gray-400">Full Name</label>
            <input 
              required
              name="name"
          ...
          <div className="flex flex-col">
            <label className="text-[16px] font-medium mb-2 text-gray-400">Email Address</label>
            <input 
              required
              type="email"
              name="email"
          ...
          <div className="flex flex-col ">
            <label className="text-[16px] font-medium mb-2  text-gray-400">Phone Number (with +CountryCode)</label>
            <input 
              required
              name="phone"
              className="input-field" 
          ...
          <div className="flex flex-col ">
            <label className="text-[16px] font-medium mb-2 text-gray-400">Institution Type</label>
            <div className="flex space-x-4 gap-4">
              ...
              <label 
                ...
                onClick={() => setFormData({...formData, category: 'College'})}
              >
                College
              </label>
            </div>
