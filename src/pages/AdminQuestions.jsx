...
import { Plus, Edit2, Trash2, X, Check, Globe, Layout, LogOut, Upload, Download, RefreshCcw } from 'lucide-react';
import { Link, useNavigate } from 'react-router-dom';
import { getAdminQuestions, addQuestion, updateQuestion, deleteQuestion, addBulkQuestions } from '../api';
...
const AdminQuestions = ({ onLogout }) => {
  ...
  const downloadSampleExcel = () => {
    const data = [
      ["Question", "Option A", "Option B", "Option C", "Option D", "Correct (1-4)", "Language", "Category"],
      ["What is the capital of France?", "London", "Paris", "Berlin", "Madrid", 2, "English", "Both"],
      ["2 + 2 = ?", "3", "4", "5", "6", 2, "English", "School"],
      ["HTML-ல் ஒரு ஹைப்பர்லிங்கை உருவாக்க எந்த டேக் பயன்படுத்தப்படுகிறது?", "<link>", "<a>", "<div>", "<img>", 2, "Tamil", "Both"]
    ];
    const ws = XLSX.utils.aoa_to_sheet(data);
    
    // Set column widths
    const wscols = [
      {wch: 40}, {wch: 20}, {wch: 20}, {wch: 20}, {wch: 20}, {wch: 15}, {wch: 10}, {wch: 10}
    ];
    ws['!cols'] = wscols;
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (formattedQuestions.length > 0) {
          ...
          showPopup("success", "Import Successful", `Imported ${formattedQuestions.length} questions`);
        } else {
          showPopup("error", "Import Failed", "No valid questions found in Excel");
        }
      } catch (err) {
...
//     // }
//     <button
//   onClick={() => setDeleteConfirm({ show: true, id: q._id })}
...
//   };
  ...
  {popup.show && (
    ...
    >
      <motion.div
        ...
        className={`relative flex items-center w-[80%] md:w-[30%] h-[15%] px-10 gap-4 px-6 py-5 rounded-xl shadow-xl text-white font-bold
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-6">
            ...
            >
              ...
              <div className="p-4 flex justify-between border rounded-t-xl border-indigo-500/30 mt-6 items-center border-b border-white/5 bg-slate-900/40 backdrop-blur-md">
                <h2 className="text-3xl font-black text-white">{editingId ? 'Edit Question' : 'Add New Question'}</h2>
                <button
                ...
                <div className="p-8 overflow-y-auto flex-1 custom-scrollbar space-y-8 border border-indigo-500/30 mb-20 rounded-b-xl">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="flex flex-col">
                      <label className="text-sm font-bold mb-3 text-gray-400 uppercase tracking-tighter mb-4">Assessment Language</label>
                      <select
                    ...
                    <div className="flex flex-col">
                      <label className="text-sm font-bold mb-4 text-gray-400 uppercase tracking-tighter">Target Category</label>
                      <select
                        className="input-field h-14"
                  ...
                  <div className="flex flex-col">
                    <label className="text-sm font-bold mb-3 text-gray-400 uppercase tracking-tighter mt-8 mb-4">Question Statement</label>
                    <textarea
                      required
                    ...
                    {formData.options.map((opt, idx) => (
                      <div key={idx} className="flex flex-col group">
                        <label className="text-sm font-bold mb-4 text-gray-400 flex justify-between items-center">
                          <span className="group-hover:text-indigo-400 transition">Option {String.fromCharCode(65 + idx)}</span>
                          <div className="flex items-center space-x-2">
                            <span className="text-[10px] opacity-0 group-hover:opacity-100 transition mr-2 uppercase border border-indigo-500/30 px-2 py-2 rounded-xl tracking-widest text-indigo-400">Mark Correct</span>
                            <input
                              type="radio"
                              name="correct"
  ...
  <button type="submit" className="btn-primary cursor-pointer flex-1 py-2 text-xl font-black shadow-2xl">
    Publish Question
  </button>

  <button type="button" onClick={() => setIsModalOpen(false)} className=" border px-8 py-2 rounded-xl border-glass-border font-bold text-sm no-underline cursor-pointer text-white hover:bg-slate-700 transition ms-4">
    Cancel
  </button>
</div>
                </div>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-6">
            ...
            >
              <div className="py-4 px-4 flex justify-between items-center border border-bottom--purple-500/30 bg-slate-900/40 backdrop-blur-md">
                <h2 className="md:text-3xl font-medium font-black text-white">Bulk Question Import</h2>
                <button
              ...
              <div className="p-12 flex flex-col items-center flex-1 justify-center max-w-2xl mx-auto w-full text-center">
                ...
                <h3 className="md:text-28px font-medium font-black mb-4">Excel/CSV Bulk Upload</h3>
                <p className="text-gray-400 mb-10 leading-relaxed md:text-[16px] font-medium">
                  To import questions in bulk, please use our standardized Excel template. 
                  Ensure your file includes columns for the <strong>Question, 4 Options, Correct Index (1-4), Language, and Category.</strong>
                </p>
                ...
                <div className="hidden md:block mt-12 p-8 bg-slate-950/50 rounded-3xl border border-glass-border w-full text-left">
                  <h4 className="text-xs font-black uppercase tracking-widest text-indigo-400 mb-4">Template Requirements</h4>
                  <ul className="space-y-3 text-sm text-gray-400">
                    <li className="flex items-start"><Check size={14} className="mr-2 mt-0.5 text-green-500" /> Correct column must be a number 1-4</li>
                    <li className="flex items-start"><Check size={14} className="mr-2 mt-0.5 text-green-500" /> Categories supported: School, College, Both</li>
                    <li className="flex items-start"><Check size={14} className="mr-2 mt-0.5 text-green-500" /> Languages supported: English, Tamil</li>
                  </ul>
                </div>
              </div>
              ...
              <div className="p-8 py-6 border-t border-white/5 bg-slate-900/80 backdrop-blur-md flex justify-center">
                <button onClick={() => setIsBulkModalOpen(false)} className="px-12 py-4 bg-slate-800 rounded-2xl font-black text-white hover:bg-slate-700 transition border border-glass-border">
                  Close Import Window
                </button>
              </div>
            </motion.div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12">
        <div>
          ...
          <div className="flex gap-6 mt-4">
            <p className="text-white text-[16px] ">Total {questions.length} questions across all target categories.</p>
           <div
  role="button"
  tabIndex={0}
...
>
  ...
  />
        ...
        <div className="flex flex-wrap gap-4 mt-8 md:mt-0">
          <Link to="/admin" className=" px-8 py-2 bg-slate-800 rounded-xl border border-glass-border font-bold text-sm no-underline text-white hover:bg-slate-700 transition flex items-center">
            Dashboard
          </Link>
          <button onClick={handleLogout} className="py-2 px-8 bg-red-500/10 text-red-500 cursor-pointer rounded-xl hover:bg-red-500 hover:text-white transition flex items-center border border-red-500/20 font-bold text-sm">
            <LogOut size={18} className="mr-2" />
            Logout
          </button>
          <button onClick={() => setIsBulkModalOpen(true)} className="cursor-pointer px-8 py-2 bg-indigo-500/10 text-indigo-400 rounded-xl hover:bg-indigo-600 hover:text-white transition flex items-center border border-indigo-500/20 font-bold text-sm">
            <Upload size={18} className="mr-2" />
            Bulk Add
          </button>
          <button onClick={() => handleOpenModal()} className="cursor-pointer btn-primary py-3 px-8 flex items-center font-bold">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 bg-slate-900/30 p-8 rounded-3xl border border-glass-border backdrop-blur-sm">
        <div className="flex flex-col">
          <label className="text-[10px] font-medium font-black uppercase tracking-widest text-indigo-400 mb-4 ml-1">Search Keywords</label>
          <input 
            type="text" 
        ...
        <div className="flex flex-col">
          <label className="text-[10px] font-black uppercase tracking-widest font-medium text-indigo-400 mb-4 ml-1">Category Filter</label>
          <select 
            className="input-field h-14 "
        ...
        <div className="flex flex-col">
          <label className="text-[10px] font-black uppercase tracking-widest text-indigo-400 font-medium mb-4 ml-1">Language Filter</label>
          <select 
            className="input-field h-14"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:gap-8">
        {filteredQuestions.length === 0 ? (
          <div className="col-span-full py-20 text-center glass-card border-dashed">
             ...
             <h3 className="text-xl font-bold text-gray-500">No questions match your current filters.</h3>
             <button onClick={() => { setSearchTerm(''); setCategoryFilter('All'); setLanguageFilter('All'); }} className="mt-4 text-indigo-400 font-bold hover:underline">Clear all filters</button>
          </div>
            ...
            <div>
              <div className="flex justify-between items-start mb-6">
                <div className="flex gap-6">
                  <span className={q.language === 'English' ? 'badge-college' : 'badge-school'}>
                    {q.language}
                  </span>
                ...
                <div className="flex gap-4 opacity-0 group-hover:opacity-100 transition ms-4 md:ms-0">
                  <button onClick={() => handleOpenModal(q)} className="px-4 cursor-pointer py-2 bg-slate-800/50 hover:bg-indigo-600 rounded-xl text-gray-400 hover:text-white transition">
                    <Edit2 size={16} />
                  </button>
                 <button
  onClick={() => setDeleteConfirm({ show: true, id: q._id })}
...
>
  <Trash2 size={16} />
              ...
              <h4 className="md:text-xl text-[16px] font-medium mb-6 leading-snug group-hover:text-indigo-300 transition md:h-[50px]">{q.text}</h4>
              <ul 
  className="mb-4 text-sm"
  style={{ display: "flex", flexDirection: "column", gap: "12px" }}
>
                {q.options.map((opt, idx) => (
                  <li key={idx} className={`py-2 px-4 rounded-xl flex items-center border ${idx === q.correct_option ? 'bg-green-500/10 border-green-500/30 text-green-400 font-bold' : 'bg-slate-900/40 border-glass-border text-gray-400'}`}>
                    <div className={`w-6 h-6 rounded-lg me-4 flex items-center justify-center text-xs font-black ${idx === q.correct_option ? 'bg-green-500 text-white' : 'bg-slate-800'}`}>
                      {String.fromCharCode(65 + idx)}
                    </div>
                    {opt}
                    {idx === q.correct_option && <Check size={16} className="ml-auto" />}
                  </li>
                ))}
              </ul>
            </div>
